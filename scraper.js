...
// restringe Atlas3 a /project/<slug> del mismo proyecto.
...
const SHEET_CSV_URL   = process.env.SHEET_CSV_URL || "";   // opcional
...
/* ===== Helpers de fuentes ===== */
async function readLocalTxt(){ if(!existsSync("sources.txt")) return []; const raw=await fs.readFile("sources.txt","utf8"); return parseTxt(raw); }
async function readRemoteTxt(url){ if(!url) return []; const r=await fetch(url); if(!r.ok) return []; return parseTxt(await r.text()); }
function parseTxt(raw){
  const out=[]; for(const line of raw.split(/\r?\n/)){ const s=line.trim();
    if(!s||s.startsWith("#")) continue;
    if(s.includes("|")){ const [name,url]=s.split("|").map(x=>x.trim()); if(isUrl(url)) out.push({name:name||url,url}); }
    else { if(isUrl(s)) out.push({name:s,url:s}); }
  } return out;
}
async function readSheetCsv(url){
  if(!url) return []; const r=await fetch(url); if(!r.ok) return [];
  const out=[], csv=await r.text(), lines=csv.split(/\r?\n/).filter(Boolean);
  const hasHeader=/name\s*,\s*url/i.test(lines[0])||/url/i.test(lines[0]); const start=hasHeader?1:0;
  for(let i=start;i<lines.length;i++){ const parts=lines[i].trim().split(",");
    if(!parts[0]) continue;
    if(parts.length>=2){ const name=parts[0].trim(); const url=parts.slice(1).join(",").trim(); if(isUrl(url)) out.push({name:name||url,url}); }
    else { const url=parts[0].trim(); if(isUrl(url)) out.push({name:url,url}); }
  } return out;
}
function isUrl(s){ try{ new URL(s); return true; }catch{ return false; } }
function dedupeSources(arr){ const seen=new Set(), out=[]; for(const it of arr){ if(!seen.has(it.url)){ seen.add(it.url); out.push(it);} } return out; }

/* ===== Estado ===== */
let state; try{ state=JSON.parse(await fs.readFile("state.json","utf8")); }catch{ state={seen:{}}; }
...
const isRaffleUrl = (raw) => {
  try{
    const u=new URL(raw), h=u.hostname.toLowerCase(), p=u.pathname.toLowerCase();

    if(h.includes("alphabot.app")){
      // descartar genéricos/estáticos
      ...
      // slugs planos: /poply-otters-testnet-mint-33xye5
    ...
    if(h.includes("subber.xyz")){
      return /(allowlist|wallet-collection|posts|campaign|raffle|giveaway)/i.test(p) && !hasFileExt(p);
    }

    return false;
  }catch{ return false; }
};
const isAlphabotProject = (u)=>{ try{ const x=new URL(u); return x.hostname.includes("alphabot.app") && x.pathname.startsWith("/_/"); }catch{ return false; } };
...
async function pickMeta(page){
  ...
  return { title:(ogTitle||title||"Nuevo sorteo").slice(0,240), description: ogDesc ? ogDesc.slice(0,1900) : "" };
}

async function postToDiscord({sourceName,url,meta}){
  if(KEYWORDS){
    const hay=`${meta?.title||""} ${meta?.description||""} ${url}`;
    if(!(new RegExp(KEYWORDS,"i")).test(hay)) return;
  }
  const payload={ username:"Giveaways Relay", content: mentionContent || undefined,
    embeds:[{ title: meta?.title||"Nuevo sorteo", url, description: (meta?.description?meta.description+"\n\n":"")+`📌 Fuente: **${sourceName}**`, timestamp: new Date().toISOString() }]
  };
...
async function scanCurrentPage(page){
  ...
  await autoScroll(page, 4); // carga cards perezosas
...
async function alphabotClickEnterButtons(page, max=8){
  const found=new Set();
  const selectors=['button:has-text("ENTER")','a:has-text("ENTER")','[role="button"]:has-text("ENTER")','button:has-text("Enter")','a:has-text("Enter")'];
  ...
  for(let i=0;i<total;i++){
    ...
    try{
      ...
      if(ok){
        ...
        await page.goBack({waitUntil:"domcontentloaded"}); try{ await page.waitForLoadState("networkidle",{timeout:6000}); }catch{}
      }else{
        ...
        if(ok2){ const u=page.url(); if(isRaffleUrl(u)) found.add(u);
          await page.goBack({waitUntil:"domcontentloaded"}); try{ await page.waitForLoadState("networkidle",{timeout:6000}); }catch{} }
      }
    }catch{}
  }
...
function looksPromisingPath(p){ const s=p.toLowerCase(); return s.includes("/project/")||s.includes("/giveaway")||s.includes("/r/")||s.includes("/raffle")||s.startsWith("/_/"); }

function atlasProjectSlug(url){
  try{ const u=new URL(url); if(!u.hostname.includes("atlas3.io")) return null;
    const m=u.pathname.match(/^\/project\/([^/]+)/i); return m?m[1].toLowerCase():null;
  }catch{ return null; }
}

async function collectCandidates(ctx, baseUrl){
  const page=await ctx.newPage();
  const candidates=new Set(); const visited=new Set();
